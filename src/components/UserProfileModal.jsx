...
export default function UserProfileModal({ isOpen, onClose, profile, setProfile, userRole, onRoleChange, isMandatory, onSave }) {
            ...
            <div className="bg-panel-raised border border-panel-line w-full max-w-md rounded-2xl shadow-glass overflow-auto max-h-[90vh] text-text-1">
                <div className="p-5 border-b border-panel-line flex justify-between items-center sticky top-0 bg-panel-raised/90 backdrop-blur-md z-10">
                    <h2 className="font-semibold text-lg flex items-center gap-2">
                        <User className="text-citizen-primary" /> Profile Settings
                    </h2>
                    {!isMandatory && (
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="col-span-2 flex flex-col">
                                <label className="block text-xs font-medium text-text-1 mb-2">Account Type</label>
                                <div className="flex bg-panel p-1 rounded-xl border border-panel-line">
                                    <button
                                        onClick={() => onRoleChange("citizen")}
                                        className={`flex-1 px-4 py-2 text-sm font-semibold rounded-lg transition-all ${userRole === "citizen" ? "bg-citizen-primary text-white shadow-sm" : "text-text-2 hover:text-text-1 hover:bg-panel-raised"}`}
                                    >
                                        Citizen
                                    </button>
                                    <button
                                        onClick={() => onRoleChange("official")}
                                        className={`flex-1 px-4 py-2 text-sm font-semibold rounded-lg transition-all ${userRole === "official" ? "bg-brass text-brass-ink shadow-sm" : "text-text-2 hover:text-text-1 hover:bg-panel-raised"}`}
                                    >
                                        Official
                                    </button>
                                </div>
                    ...
                    {userRole === "citizen" ? (
                        <>
                            <div className="space-y-6">
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-text-1 mb-2">1. Main health priority?</label>
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-text-1 mb-2">2. What to check on labels?</label>
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-text-1 mb-2">3. Food restrictions?</label>
                                    <div className="flex flex-wrap gap-2 text-text-2">
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-text-1 mb-2">4. Health-consciousness level?</label>
                            ...
                            <hr className="border-panel-line" />
                            <div className="space-y-4">
                                <h3 className="text-sm font-semibold text-status-fail uppercase tracking-wider flex items-center gap-1.5"><Activity size={14} /> Medical Records</h3>
                                <div className="bg-panel p-4 rounded-xl border border-panel-line mb-4">
                                    <label className="flex items-center gap-2 text-sm font-medium cursor-pointer">
                                        <input type="checkbox" checked={profile.hasDiabetes || false} onChange={(e) => handleSimpleChange('hasDiabetes', e.target.checked)} className="w-4 h-4 rounded border border-panel-line checked:bg-status-fail checked:border-status-fail transition-colors" />
                                        <span>I have Diabetes (Alert for high sugar/carbs)</span>
                                    </label>
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-text-1 mb-2">Known Allergies</label>
                                    <div className="flex flex-wrap gap-2 text-text-2">
                        ...
                        <div className="space-y-4">
                            <h3 className="text-sm font-semibold text-text-2 uppercase tracking-wider flex items-center gap-1.5"><Activity size={14} /> Official Credentials</h3>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="col-span-2">
                                    <label className="block text-xs font-medium text-text-1 mb-1">Department</label>
                                    <select name="department" value={profile.department || ''} onChange={handleChange} className="w-full bg-panel text-text-1 border border-panel-line rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brass/60">
                                ...
                                <div className="col-span-2">
                                    <label className="block text-xs font-medium text-text-1 mb-1">Official ID Number</label>
                                    <input type="text" name="officialId" value={profile.officialId || ''} onChange={handleChange} className="w-full bg-panel text-text-1 border border-panel-line rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brass/60" placeholder="e.g. INSP-2026-X" />
                                </div>
                                <div className="col-span-2">
                                    <label className="block text-xs font-medium text-text-1 mb-1">Region / Zone</label>
                                    <input type="text" name="region" value={profile.region || ''} onChange={handleChange} className="w-full bg-panel text-text-1 border border-panel-line rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brass/60" placeholder="e.g. West Zone, Mumbai" />
                                </div>
                            </div>
                        </div>
                    )}
                ...
                <div className="p-5 border-t border-panel-line bg-panel-darker rounded-b-2xl flex justify-end">
                    <button onClick={onSave} className="px-5 py-2.5 bg-citizen-primary text-white font-semibold rounded-lg shadow-md hover:shadow-lg hover:bg-citizen-secondary transition-all flex items-center gap-2 text-sm">
                        <Check size={16} /> Save Profile & Enter
                    </button>
                </div>
            </div>
        </div>
